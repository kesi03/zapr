...
export const uploadGraphQlCommand: yargs.CommandModule = {
  command: 'upload-graphql <file>',
  describe: 'Upload a GraphQL schema (.graphql/.gql/.yaml/.yml/.json) to ZAP',
  builder: (yargs) => {
    return yargs
      .positional('file', {
        type: 'string',
        describe: 'Path to the GraphQL schema file',
        demandOption: true,
      })
      .option('name', {
        alias: 'n',
  ...
  handler: async (argv) => {
    ...
    try {
      ...
      const result = await zap.configUploader.uploadGraphQl(fileName, fileContent, argv.overwrite as boolean);
