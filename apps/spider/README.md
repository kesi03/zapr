## Spider (TypeScript) — capture screenshots + build sitemap JSON
...
Usage:
- Install: npm install
- Run: npx ts-node src/cli.ts --baseUrl=https://example.com --out=output --concurrency=3

package.json
```json
{
  "name": "pw-sitemap-screenshots",
  "version": "1.0.0",
  "main": "src/cli.ts",
  "scripts": {
...
Notes:
- Captures front page then crawls same-origin links (BFS) up to maxPages.
- Saves screenshots under <out>/screenshots and sitemap at <out>/sitemap.json.
- Adjust timeouts, viewport, concurrency as needed.
- Ensure you have permission to crawl/photograph the site.